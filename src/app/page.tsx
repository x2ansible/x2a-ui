...
const workflows = [
  {
    ...
    title: "x2Ansible",
    description:
      "Transform compatible IAC into Ansible Playbooks in a guided approach.",
        ...
        >
          {workflows.map((wf, idx) => (
            <div
              key={wf.key}
              className={`
                relative group bg-gradient-to-br ${wf.cardColor} border backdrop-blur-lg rounded-2xl shadow-xl px-5 py-7 sm:px-6 sm:py-8 md:px-7 md:py-9 flex flex-col items-center text-center gap-2 transition-all duration-300 hover:scale-105 hover:shadow-2xl hover:shadow-blue-500/20 hover:border-opacity-80 hover:brightness-110 animate-fade-up
                ${wf.comingSoon ? "cursor-not-allowed opacity-80 hover:opacity-90" : "cursor-pointer hover:cursor-pointer"}
            ...
            >
              ...
              <h2 className="text-lg md:text-xl font-extrabold text-white mb-1 tracking-tight group-hover:text-blue-200 transition-colors duration-300">
                {wf.title}
              </h2>
              <p className="text-slate-200 text-[15px] md:text-base font-medium mb-1 max-w-xs mx-auto opacity-90 group-hover:opacity-100 group-hover:text-slate-100 transition-all duration-300">
                {wf.description}
              </p>
              ...
              <button
                ...
                className={`mt-3 px-5 py-2 text-[15px] md:text-base font-bold rounded-lg bg-gradient-to-r ${wf.buttonColor} shadow ring-1 ring-white/10 hover:shadow-lg hover:shadow-blue-400/30 hover:brightness-110 hover:scale-105 hover:ring-2 hover:ring-blue-300/50 transition-all duration-200 text-white focus:outline-none focus:ring-4 focus:ring-blue-300 w-full transform active:scale-95`}
              ...
              {wf.comingSoon && (
                <span className="absolute top-3 right-3 text-xs bg-slate-700/80 text-slate-100 px-2 py-1 rounded-full shadow group-hover:bg-slate-600/90 transition-colors duration-300">
                  Coming soon
                </span>
              )}
            </div>
      ...
      {pendingWorkflow && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="relative bg-gradient-to-br from-slate-900/90 to-slate-800/80 border border-slate-700/80 rounded-2xl shadow-2xl p-8 w-full max-w-md flex flex-col items-center animate-fade-in">
            <button
              className="absolute top-4 right-4 text-slate-400 hover:text-slate-100 hover:scale-110 transition-all duration-200"
              onClick={closeModal}
              aria-label="Close"
            >
            ...
            <button
              className="w-full flex items-center justify-center gap-3 px-6 py-3 bg-gradient-to-r from-gray-800 to-slate-700 border border-slate-600 rounded-xl font-semibold text-white text-lg shadow hover:bg-slate-700 hover:border-slate-500 hover:shadow-lg hover:scale-105 transition-all duration-200 mb-2 active:scale-95"
            ...
            <button
              className="mt-1 px-4 py-1 bg-transparent text-slate-400 hover:text-white hover:underline text-sm transition-all duration-200"
              onClick={closeModal}
            >
              Cancel
            </button>
