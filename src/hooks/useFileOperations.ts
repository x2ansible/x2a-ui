...
export const useFileOperations = ({
  ...
  const fetchFolders = useCallback(async () => {
    try {
      ...
      const data: FileListResponse = await apiCall("/api/files/list");
      if (data.error) throw new Error(data.error);

      const unique = Array.from(
        new Set(data.folders!.filter(folder => folder !== gitRepoName))
      );
  ...
  const handleUpload = useCallback(async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      addLogMessage(` Upload complete. Ready for classification.`);
    } catch (error) {
