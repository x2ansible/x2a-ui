...
def set_upload_dir(upload_dir: str):
    ...
    # 🔧 ONLY ADDED: Try to set write permissions (safe if it fails)
    try:
...
async def upload_files(files: List[UploadFile] = File(...)):
    # 🔧 ONLY ADDED: Ensure directory exists with permissions
    os.makedirs(UPLOAD_DIR, exist_ok=True)
    ...
    for file in files:
        ...
        # 🔧 ONLY CHANGED: Add try/catch for better error message
        try:
...
# 🔧 EVERYTHING BELOW IS 100% UNCHANGED - ALL YOUR WORKING FUNCTIONALITY STAYS IDENTICAL
